...
interface SegmentComparisonChartProps {
  segments: Segment[];
  metric: 'userCount' | 'percentageOfTotal' | 'avgPurchaseValue' | 'conversionRate';
  title?: string;
}
...
export const SegmentComparisonChart: React.FC<SegmentComparisonChartProps> = ({ 
  ...
  const data = segments.map(segment => {
    const additionalMetrics = calculateMetrics(segment);
    ...
    <div className="w-full h-full">
      ...
      <ResponsiveContainer width="100%" height={300}>
        ...
        >
          ...
          <YAxis />
          <Tooltip formatter={(value) => [getTooltipFormatter(value), getMetricLabel()]} />
          <Legend />
